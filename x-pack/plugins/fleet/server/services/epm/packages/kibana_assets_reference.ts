...
import type { KibanaAssetReference, Installation } from '../../../types';
 ...
 */
export const updateKibanaAssetReferences = async (
  savedObjectsClient: SavedObjectsClientContract,
  pkgName: string,
  currentAssets: KibanaAssetReference[],
  {
    assetsToAdd = [],
    assetsToRemove = [],
    refresh = false,
  }: {
    assetsToAdd?: KibanaAssetReference[];
    assetsToRemove?: KibanaAssetReference[];
    /**
  ...
  const {
    attributes: { installed_kibana: updatedAssets },
  } =
    ...
    await pRetry(
      () =>
        savedObjectsClient.update<Installation>(
          ...
          pkgName,
          {
            installed_kibana: deduplicatedAssets,
          },
          {
            refresh,
          }
